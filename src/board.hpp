...
class Board {
public:
  Board(sf::Vector2i size = {15, 15})
      : size_(size), max_steps_(size.x * size.y) {
    reset();
  }
  ...
  void draw(sf::RenderWindow &window) {
    for (int y = 1; y <= size_.y; y++) {
      sf::RectangleShape line(
          sf::Vector2f((size_.x - 1) * chess_offset_, line_thickness_));
      line.setOrigin(0, line.getSize().y / 2);
      line.setPosition(chess_offset_, y * chess_offset_);
    ...
    for (int x = 1; x <= size_.x; x++) {
      sf::RectangleShape line(
          sf::Vector2f(line_thickness_, (size_.y - 1) * chess_offset_));
      line.setOrigin(line.getSize().x / 2, 0);
      line.setPosition(x * chess_offset_, chess_offset_);
      line.setFillColor(sf::Color::Black);
      window.draw(line);
    }
  ...
  Chess get_chess(sf::Vector2i position) const {
...
private:
  ...
  void draw_chess(sf::RenderWindow &window, sf::Vector2i position,
                  Chess chess) {
    sf::CircleShape chess_shape(chess_diameter_ / 2.f, 50);
    chess_shape.setOrigin(chess_shape.getRadius(), chess_shape.getRadius());
    chess_shape.setPosition(chess_offset_ + position.x * chess_offset_,
                            chess_offset_ + position.y * chess_offset_);
  ...
  void draw_stars(sf::RenderWindow &window) {
    ...
    star.setPosition(3 * chess_offset_ + chess_offset_,
                     3 * chess_offset_ + chess_offset_);
    window.draw(star);
    star.setPosition((size_.x - 4) * chess_offset_ + chess_offset_,
                     3 * chess_offset_ + chess_offset_);
    window.draw(star);
    star.setPosition(3 * chess_offset_ + chess_offset_,
                     (size_.y - 4) * chess_offset_ + chess_offset_);
    window.draw(star);
    star.setPosition((size_.x - 4) * chess_offset_ + chess_offset_,
                     (size_.y - 4) * chess_offset_ + chess_offset_);
    window.draw(star);
